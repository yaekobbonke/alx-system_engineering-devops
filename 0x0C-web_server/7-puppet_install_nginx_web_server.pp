# install nginx on new server with puppet
exec {'/usr/bin/env apt-get -y update': }
exec {'/usr/bin/env apt-get -y install nginx': }
exec {'/usr/bin/env echo "Hello World!" > /var/www/html/index.nginx-debian.html': }
exec {'/usr/bin/env sed -i "/server_name _;/ a rewrite ^/redirect_me http://www.HelloWorld!.com permanent;" /etc/nginx/sites-available/default': }
